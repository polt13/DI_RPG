...
static bool spawned = false;  // keep track of whether or not heroes spawned, to
                              // avoid immediately entering battle

Market::Market(const std::vector<Weapon*>& AllWeapons,
               const std::vector<Armor*>& AllArmors,
               const std::vector<Potion*>& AllPotions,
               const std::vector<Spell*>& AllSpells)
    : size(5),
...
void Market::buy(Hero* MyHero, itemType itype, int index) {
  if (itype == itemType::WEAPON) {
    if (weapons[index]->getPrice() <= MyHero->getMoney()) {
      std::cout << "\n\n";
      std::cout << "\tPurchased " << weapons[index]->get_name()
                << " Weapon for " << MyHero->get_name() << "\n";
    ...
    if (armors[index]->getPrice() <= MyHero->getMoney()) {
      std::cout << "\n\n";
      std::cout << "\tPurchased " << armors[index]->get_name() << " Armor for "
                << MyHero->get_name() << "\n";
    ...
    if (potions[index]->getPrice() <= MyHero->getMoney()) {
      std::cout << "\n\n";
      std::cout << "\tPurchased " << potions[index]->get_name()
                << " Potion for " << MyHero->get_name() << "\n";
    ...
    if (spells[index]->getPrice() <= MyHero->getMoney()) {
      std::cout << "\n\n";
      std::cout << "\tPurchased " << spells[index]->get_name() << " Spell for "
                << MyHero->get_name() << "\n";
...
void Market::acquire(Weapon* MyWeapon) { weapons.push_back(MyWeapon); }

void Market::acquire(Armor* MyArmor) { armors.push_back(MyArmor); }

void Market::acquire(Potion* MyPotion) { potions.push_back(MyPotion); }

void Market::acquire(Spell* MySpell) { spells.push_back(MySpell); }
...
blockType Block::get_type() const { return Btype; }

// no market in common
Common::Common() : Block(blockType::COMMON) {}

void Common::move(std::vector<Hero*>& toMove) {
  Squad.insert(
      Squad.begin(), toMove.begin(),
      toMove.end());  // copy heroes to new location (this common block)
  toMove.clear();     // empty previous block
  if (spawned ==
      true) {  // dont attempt to put in battle immediately after spawning
    if (std::rand() % 100 > 80) {
      std::cout << "\n\n";
...
void Common::interact_with() {
  ...
  auto monster_level =
      Squad[0]->getLevel();  // monster level based on hero level
...
void Common::fight(std::vector<Monster*>& enemies) {
  ...
  while (end_fight(enemies) ==
         false) {  // checks if condition to finish battle are met
    // round starts here
    int option;
    do {
      std::cout << "\n\n1 to view battle status, 2 to view inventory, 3 to "
                   "attack, 4 to escape\n\n";
      std::cin >> option;
      Game::clearbuffer();
    ...
    if (option == 1) {
      ...
      std::cout << "Check inventory of character? [ 1 to " << Squad.size()
                << "]\n";
      int HeroPick;
      while (!(std::cin >> HeroPick) || HeroPick > Squad.size() ||
             HeroPick < 1) {
        std::cout << "Not a valid hero\n";
        Game::clearbuffer();
      }
      ...
      while (!(std::cin >> pick) || pick > 3 || pick < 1) {
      ...
      switch (pick) {
        ...
        case 2:
          ...
          while (!(std::cin >> whichSpell) || whichSpell < 1 ||
                 (whichSpell > Squad[HeroPick]->inv_size(itemType::SPELL))) {
            std::cout << "Not a valid spell\n";
          }
          battle_status(enemies);
          std::cout << " Use spell on: [1 to " << enemies.size() << "]\n";
          while (!(std::cin >> enemyTarget) || enemyTarget < 1 ||
                 (enemyTarget > enemies.size())) {
            std::cout << "Not a valid enemy\n";
          }
          --enemyTarget;
          Squad[HeroPick]->castSpell(enemies[enemyTarget], whichSpell - 1);
          if (enemies[enemyTarget]->get_hp() == 0) {
            delete enemies[enemyTarget];  // if monster dead after attack remove
                                          // it
      ...
      int enemy_at;
      battle_status(enemies);
      std::cout << Squad[atk_hero]->get_name() << " attacks: [1 to "
                << enemies.size() << "]\n";
      while (!(std::cin >> enemy_at) || (enemy_at > enemies.size()) ||
             enemy_at < 1) {
        std::cout << "No such enemy\n";
      }
      --enemy_at;
      Squad[atk_hero++]->attack(
          enemies[enemy_at]);  // attack and go to next hero
      if (enemies[enemy_at]->get_hp() == 0) {
        ...
        if (enemies.size() == 0) continue;  // fight needs to end
      }
    ...
    auto heroTarget =
        std::rand() % Squad.size();  // randomly pick hero target for monster AI
    std::cout << enemies[atk_monster]->get_name() << " attacks "
              << Squad[heroTarget]->get_name() << '\n';
  ...
  for (auto h : Squad) {  // find if all heroes dead
    if (h->get_hp() != 0) {
      ...
      std::cout << h->get_name() << " got " << xp_receive << "XP, "
                << gold_receive << " in gold!\n";
...
bool Common::end_fight(const std::vector<Monster*>& enemies) {
  ...
  if (enemies.size() == 0) monsters_dead = true;

  for (const auto h : Squad) {
    if (h->get_hp() != 0) heroes_dead = false;  // at least one alive
  }
...
Inaccessible::Inaccessible() : Block(blockType::INACCESSIBLE) {
  //
}

void Inaccessible::move(std::vector<Hero*>& toMove) { interact_with(); }
...
void Inaccessible::print() const { std::cout << "  #  "; }
  ...
  if (input == 0)
    ...
    switch (input) {
      case 1:
        ...
        while (!(std::cin >> index) || index <= 0 ||
               index >= h->inv_size(itemType::WEAPON) + 1) {
          std::cout << "\n";
          std::cout << std::setw(50) << "Invalid input (Must be: 1 - "
                    << h->inv_size(itemType::WEAPON) << ")\n";
          Game::clearbuffer();
      ...
      case 2:
        ...
        while (!(std::cin >> index) || index <= 0 ||
               index >= h->inv_size(itemType::ARMOR) + 1) {
          std::cout << "\n";
          std::cout << std::setw(50) << "Invalid input (Must be: 1 - "
                    << h->inv_size(itemType::ARMOR) << ")\n";
          Game::clearbuffer();
          std::cout << std::setw(37) << "Sell which item? (index from 1)\n";
        }
        index--;
      ...
      case 3:
        ...
        while (!(std::cin >> index) || index <= 0 ||
               index >= h->inv_size(itemType::POTION) + 1) {
          std::cout << "\n";
          std::cout << std::setw(50) << "Invalid input (Must be: 1 - "
                    << h->inv_size(itemType::POTION) << ")\n";
          Game::clearbuffer();
          std::cout << std::setw(37) << "Sell which item? (index from 1)\n";
        }
        index--;
      ...
      case 4:
        ...
        while (!(std::cin >> index) || index <= 0 ||
               index >= h->inv_size(itemType::SPELL) + 1) {
          std::cout << "\n";
          std::cout << std::setw(50) << "Invalid input (Must be: 1 - "
                    << h->inv_size(itemType::SPELL) << ")\n";
          Game::clearbuffer();
          std::cout << std::setw(37) << "Sell which item? (index from 1)\n";
        }
        index--;
